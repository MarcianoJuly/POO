...
public class Cenario
{
    ...
    {          
        ...
        try
        {
            //clocar o nome da nave
            nave.alterarNome();

            // carrega as imagens usadas no jogo
            imgNave = ImageIO.read(new File("src/imagens/nave.png"));
            imgInimigo = ImageIO.read(new File("src/imagens/inimigo.png"));
            imgMisselNave = ImageIO.read(new File("src/imagens/misselNave.png"));
            imgMisselInimigo = ImageIO.read(new File("src/imagens/misselInimigo.png"));
            imgExplosao = ImageIO.read(new File("src/imagens/explosao.png"));
    ...
    public void executarLoopDeJogo()
    {
        ...
        while (!jogoTerminou)
        {
            ...
            tempo = (1400/fps) - (System.currentTimeMillis() - tempo);
            if (tempo > 0)
            {
                try
                {
    ...
    {                    
        ...
        private void desenharMensagensGerais(Graphics g)
        {
            ...
            if (gameOver)
            {
                ...
                mensagem3 = "F1 para mudar de nome...";
            ...
            if (temMensagem) // mudar caso game over caso pause/menu
            {
                Dimension tamanho = getSize();
                ...
                g.fillRoundRect(posX - 30, posY - 30, 300, 80, 10, 10);
                
                g.setColor(Color.WHITE);
                g.drawString(mensagem1, posX, posY-10);
                g.drawString(mensagem2, posX, posY+10);
                g.drawString(mensagem3, posX, posY+30);
            }
    ...
    private class InputHandler implements KeyListener
    {
        ...
        private void tratarTeclasControle(int keyCode)
        {
            ...
            if (input.teclaEstaPressionada(KeyEvent.VK_F1))
            {    
                nave.alterarNome();
                inicializar();        
            }
